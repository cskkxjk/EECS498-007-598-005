...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
      for j in range(num_test):
          dists[i, j] = torch.sum(torch.square(x_train[i].view(-1) - x_test[j].view(-1)))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for i in range(num_train):
    dists[i] = torch.sum(torch.square(x_train[i].view(-1).repeat(num_test, 1) - x_test.view(num_test, -1)), dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  dists = -2 * torch.matmul(x_train.view(num_train, -1), x_test.view(num_test, -1).t()) + torch.sum(torch.square(x_train.view(num_train, -1)), dim=1).view(-1, 1) + torch.sum(torch.square(x_test.view(num_test, -1)), dim=1).view(1, -1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  nearest_neighbor_indices = torch.topk(dists, dim=0, k=k, largest=False)[1]
  nearest_neighbor_label = y_train[nearest_neighbor_indices.t()]
  for i in range(num_test):
      topk = nearest_neighbor_label[i].tolist()
      l1 = []
      for j in set(topk):
          l1.append([j, topk.count(j)])
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
      ...
      for fold_idx in range(num_folds):
          fold_val_x = x_train_folds[fold_idx]
          fold_val_y = y_train_folds[fold_idx]
          fold_train_x = torch.cat(x_train_folds[:fold_idx] + x_train_folds[fold_idx+1:])
          fold_train_y = torch.cat(y_train_folds[:fold_idx] + y_train_folds[fold_idx+1:])
          knn_cla = KnnClassifier(fold_train_x, fold_train_y)
          k_to_accuracies[k].append(knn_cla.check_accuracy(fold_val_x, fold_val_y, k=k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
