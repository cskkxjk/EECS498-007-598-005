...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # Replace "pass" statement with your code
        N, _, H, W = x.shape
        encoder_out = self.encoder(torch.cat((x.reshape(N, -1), c), dim=1))
        mu = self.mu_layer(encoder_out)
        logvar = self.logvar_layer(encoder_out)
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    z = torch.randn_like(mu) * logvar.exp().sqrt() + mu
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # Replace "pass" statement with your code
    N = x.shape[0]
    reconstruction_loss = torch.nn.functional.binary_cross_entropy(x_hat, x, reduction='none').reshape(N, -1).sum(dim=1).mean()
    # print(reconstruction_loss.shape)
    KL_divergence_loss = -0.5 * torch.mean((1 + logvar - mu ** 2 - logvar.exp()).sum(dim=1))
