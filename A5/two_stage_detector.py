...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    A = self.num_anchors
    out = self.pred_layer(features).reshape(B, A, 6, H, W)  # shape(B, (Ax6), H, W)
    conf_scores = out[:, :, :2, :, :]  # (B, A, 2, H, W)
    offsets = out[:, :, 2:, :, :]  # (B, A, 4, H, W)
    if mode == 'train':
      ...
      conf_scores = torch.cat((pos_conf_scores, neg_conf_scores), dim=0)   # (2M, 2)
      offsets = self._extract_anchor_data(offsets, pos_anchor_idx)  # (M, 4)
      proposals = GenerateProposal(pos_anchor_coord[None, None, None, :, :], offsets[None, None, None, :, :], method='FasterRCNN').reshape(-1, 4)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'train':
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    grid = GenerateGrid(B, H, W)  # B, H, W, 2
    anchors = GenerateAnchor(self.anchor_list, grid)  # B, A, H, W, 4
    # IoU and GT data
    iou_mat = IoU(anchors, bboxes)
    pos_anchor_idx, negative_anc_ind, GT_conf_scores, GT_offsets, GT_class, pos_anchor_coord, _ = \
      ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, neg_thresh=0.2, method='FasterRCNN')
    # comput conf_scores, offsets, proposals
    conf_scores, offsets, proposals = self.prop_module(features, pos_anchor_coord, pos_anchor_idx, negative_anc_ind)
    # total loss
    total_loss = w_conf * ConfScoreRegression(conf_scores, B) + w_reg * BboxRegression(offsets, GT_offsets, B)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      ...
      grid = GenerateGrid(B, H, W)  # B, H, W, 2
      anchors = GenerateAnchor(self.anchor_list, grid)  # B, A, H, W, 4
      # compute conf_scores and offsets
      conf_scores, offsets = self.prop_module(features)  # (B, A, 2, H, W) (B, A, 4, H, W)
      conf_scores = torch.sigmoid(conf_scores.permute(0, 1, 3, 4, 2)[:, :, :, :, 0])  # B, A, H, W
      ...
      for b in range(B):
        ...
        conf_proposals = proposals[b][conf_mask] # N, 4
        NMS_keep = torchvision.ops.nms(conf_proposals, conf_score, iou_threshold=nms_thresh)
        final_proposals.append(conf_proposals[NMS_keep])
        final_conf_probs.append(conf_score[NMS_keep].unsqueeze(1))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'RPN':
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    # RoI Align
    temp_boxes = torch.zeros((proposals.shape[0], 5), dtype=proposals.dtype, device=proposals.device)
    temp_boxes[:, 0] = (pos_anchor_idx // anc_per_img)
    temp_boxes[:, 1:] = proposals
    roi = torchvision.ops.roi_align(features, temp_boxes, (self.roi_output_h, self.roi_output_w))
    mean_roi = roi.mean(dim=(2, 3))
    # classification
    class_scores = self.cls_layer(mean_roi)
    cls_loss = nn.functional.cross_entropy(class_scores, GT_class)
    total_loss = rpn_loss + cls_loss
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
      ...
      # (P_i, 4), (P_i, 1), (B, D, H, W)
      # RoI Align
      roi = torchvision.ops.roi_align(features, final_proposals, (self.roi_output_h, self.roi_output_w))
      mean_roi = roi.mean(dim=(2, 3))
      # classification
      class_scores = self.cls_layer(mean_roi)
      final_class = torch.argmax(class_scores, dim=1, keepdim=True)
      idx=[]
      for proposal in final_proposals:
        idx.append(proposal.shape[0])
      final_class=torch.split(final_class, idx)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
