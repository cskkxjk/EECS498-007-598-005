...
def compute_saliency_maps(X, y, model):
  ...
  scores = model(X)
  N = X.shape[0]
  loss = (-1.0/N) * scores[torch.arange(N), y].sum()
  loss.backward()
  dX = X.grad.data
  saliency = dX.abs().max(dim=1)[0]
  ##############################################################################
  #               END OF YOUR CODE                                             #
  ##############################################################################
  return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
      score_adv = model(X_adv)
      max_score = torch.max(score_adv, dim=1)[0]
      target_score = score_adv[:, target_y]
      if max_score == target_score:
          break
      if verbose:
          print(f'Iter:{i} max_score:{max_score} target_score:{target_score}')
      loss = target_score
      loss.backward()
      g = X_adv.grad.data
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    # Replace "pass" statement with your code
    score = model(img)
    target_score = score[:, target_y]
    R = l2_reg * torch.sum(img ** 2)
    loss = target_score - R
    loss.backward()
    grad = img.grad.data
    dimg = learning_rate * grad
    img.data = img.data + dimg.data
    img.grad.data.zero_()
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
