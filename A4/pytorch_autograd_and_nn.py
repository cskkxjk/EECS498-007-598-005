...
def three_layer_convnet(x, params):
  ...
  x = F.relu(F.conv2d(x, conv_w1, conv_b1, 1, 2))
  x = F.relu(F.conv2d(x, conv_w2, conv_b2, 1, 1))
...
class ThreeLayerConvNet(nn.Module):
  ...
  def forward(self, x):
    ...
    scores = self.fc(flatten(F.relu(x)))
    ############################################################################
    #                            END OF YOUR CODE                          
    ############################################################################
    return scores
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.shortcut = nn.Identity() if Cin == Cout else nn.Conv2d(Cin, Cout, 1, stride=2 if downsample else 1)
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    seq = [ResNetStem(Cin, stage_args[0][0])]
    for i in stage_args:
      seq.append(ResNetStage(*i, block=block))
