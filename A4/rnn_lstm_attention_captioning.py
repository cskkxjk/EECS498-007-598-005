...
def rnn_backward(dh, cache):
    ...
    dh_temp = torch.zeros(N, H, device=dh.device, dtype=dh.dtype)
    for t in range(T-1, -1, -1):
        dx_t, dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh_temp + dh[:, t, :], cache[t])
        dx[:, t, :] = dx_t
        dh_temp = dprev_h_t
        dWx += dWx_t
        dWh += dWh_t
...
class WordEmbedding(nn.Module):
  ...
  def forward(self, x):
      ...
      # Replace "pass" statement with your code
      N, T = x.shape
      out = self.W_embed[x.reshape(N * T,)].reshape(N, T, -1)
      ##############################################################################
      #                               END OF YOUR CODE                             #
      ##############################################################################
      return out
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    # Replace "pass" statement with your code
    N, T, V = x.shape
    loss = F.cross_entropy(x.reshape(N * T, V), y.reshape(N * T,), ignore_index=ignore_index, reduction='none').reshape(N, T).mean(dim=0).sum()
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.featureextractor = FeatureExtractor(pooling=False if self.cell_type == 'attention' else True, device=device, dtype=dtype)
        self.affline1 = nn.Linear(input_dim, hidden_dim).to(device)
        ...
        self.affline2 = nn.Linear(hidden_dim, vocab_size).to(device)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def forward(self, images, captions):
        ...
        ############################################################################
        # Replace "pass" statement with your code
        img_feature = self.featureextractor.extract_mobilenet_feature(images)  # shape(N, D)/ (N, D, 4, 4)
        word_embed_in = self.word_embed(captions_in)
        if self.cell_type == 'attention':
            img_feature = img_feature.permute(0, 2, 3, 1)
            A = self.affline1(img_feature).permute(0, 3, 1, 2)  # shape (N, H, 4, 4)
            rnn_out = self.rnn(word_embed_in, A)
        else:
            h0 = self.affline1(img_feature)  # shape (N, H)
            rnn_out = self.rnn(word_embed_in, h0)
        temp_affline = self.affline2(rnn_out)
        loss = temporal_softmax_loss(temp_affline, captions_out, ignore_index=self.ignore_index)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == 'attention':
            ...
            raise NotImplementedError
        start_embed = self.word_embed(torch.zeros(N, 1, device=h0.device, dtype=h0.dtype).fill_(self._start).long())
        embed = start_embed
        for i in range(max_length):
            if self.cell_type == 'rnn':
                next_h = self.rnn.step_forward(embed[:, 0, :], prev_h)
            elif self.cell_type == 'lstm':
                next_h, prev_c = self.rnn.step_forward(embed[:, 0, :], prev_h, prev_c)
            elif self.cell_type == 'attention':
                attn, attn_weight = dot_product_attention(prev_h, A)
                next_h, prev_c = self.rnn.step_forward(embed[:, 0, :], prev_h, prev_c, attn)
                attn_weights_all[:, i, :, :] = attn_weight
            else:
                raise NotImplementedError
            scores = self.affline2(next_h)  # (N, V)
            caption = torch.max(scores, dim=1, keepdim=True)[1] # (N, 1)
            prev_h = next_h

            embed = self.word_embed(caption)
            captions[:, i] = caption.squeeze(1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        if self.cell_type == 'attention':
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b, attn=None, Wattn=None):
    ...
    H = prev_h.shape[1]
    if attn is not None:
        out = x.mm(Wx) + prev_h.mm(Wh) + attn.mm(Wattn) + b
    else:
...
def dot_product_attention(prev_h, A):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    A = A.reshape(N, H, -1)  # shape(N, H, 16)
    attn_weights = F.softmax(prev_h[:, None, :].bmm(A)[:, 0, :].div(H ** 0.5), dim=1)  # shape(N, 16)
    attn = A.bmm(attn_weights[:, :, None]).squeeze(-1)
