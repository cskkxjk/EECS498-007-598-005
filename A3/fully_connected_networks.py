...
class Linear(object):
  ...
  def backward(dout, cache):
    ...
    dw = cache[0].reshape(dout.shape[0], -1).t().mm(dout)
    dx = dout.mm(cache[1].t()).reshape(cache[0].shape)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx, dw, db
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dh1, grads['W2'], grads['b2'] = Linear.backward(dloss, cache2)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for layer in range(2, self.num_layers):
      self.params[f'W{layer}'] = torch.randn(hidden_dims[layer-2], hidden_dims[layer-1], dtype=dtype, device=device) * weight_scale
      self.params[f'b{layer}'] = torch.zeros(hidden_dims[layer-1], dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    hidden = list(range(self.num_layers+1))
    hidden[0] = X
    ...
    for layer in range(1, self.num_layers + 1):
      ...
      if layer == self.num_layers:
        hidden[layer], cache[layer-1] = Linear.forward(hidden[layer-1], W, b)
      else:
        hidden[layer], cache[layer - 1] = Linear_ReLU.forward(hidden[layer - 1], W, b)
        if self.use_dropout:
          hidden[layer], dropout_cache[layer-1] = Dropout.forward(hidden[layer], self.dropout_param)
    ...
    for layer in range(self.num_layers, 0, -1):
      if layer == self.num_layers:
        dhiddens[layer - 1], grads[f'W{layer}'], grads[f'b{layer}'] = Linear.backward(dhiddens[layer], cache[layer - 1])
      else:
        if self.use_dropout:
          dhiddens[layer] = Dropout.backward(dhiddens[layer], dropout_cache[layer-1])
        dhiddens[layer - 1], grads[f'W{layer}'], grads[f'b{layer}'] = Linear_ReLU.backward(dhiddens[layer], cache[layer-1])

      loss += self.reg * (self.params[f'W{layer}'] ** 2).sum()
      grads[f'W{layer}'] += 2 * self.reg * self.params[f'W{layer}']
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask = torch.rand_like(x) < dropout_param['p']
      out = x.masked_fill(mask, 0) / (1 - dropout_param['p'])
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
    elif mode == 'test':
  ...
  def backward(dout, cache):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      dx = dout.masked_fill(mask, 0) / (1 - dropout_param['p'])
      ###########################################################################
      #                            END OF YOUR CODE                             #
      ###########################################################################
    elif mode == 'test':
