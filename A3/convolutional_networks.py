...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    x_pad = torch.nn.functional.pad(x, [pad] * 4, 'constant')
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for k in range(F):
                out[:, k, i, j] = torch.sum(x_pad_mask * w[k, :, :, :], dim=[1, 2, 3])
  ...
  def backward(dout, cache):
    ...
    x_pad = torch.nn.functional.pad(x, [pad] * 4, 'constant') # shape(N, C, H+2*pad, W+2*pad)
    db = torch.sum(dout, dim=[0, 2, 3])
    dx = torch.zeros_like(x) # shape (N, C, H, W)
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_pad_mask = x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            for k in range(F):
              dw[k, :, :, :] += torch.sum(x_pad_mask * (dout[:, k, i, j])[:, None, None, None], dim=0)
            for n in range(N):
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    for i in range(H_out):
      for j in range(W_out):
        pool = x[:, :, i*stride:i*stride+height, j*stride:j*stride+width]
        out[:, :, i, j] = torch.max(pool.reshape(N, C, -1), dim=2)[0]
  ...
  def backward(dout, cache):
    ...
    for i in range(H_out):
      for j in range(W_out):
        pool = x[:, :, i * stride:i * stride + height, j * stride:j * stride + width]
        max_num = torch.max(pool.reshape(N, C, -1), dim=2, keepdim=True)[0]
        pool_mask = (pool.reshape(N, C, -1) == max_num).reshape(pool.shape)
        dx[:, :, i*stride:i*stride+height, j*stride:j*stride+width] += pool_mask * dout[:, :, i, j][:, :, None, None]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W2'] = torch.randn(int(F * H/2 * W/2), hidden_dim, dtype=dtype, device=device) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    pool_out, pool_cache = FastMaxPool.forward(conv_out, pool_param)
    linear_relu_out, linear_relu_cache = Linear_ReLU.forward(pool_out, W2, b2)
    ...
    dlinear, grads['W3'], grads['b3'] = Linear.backward(dscore, linear_cache)
    dlinear_relu, grads['W2'], grads['b2'] = Linear_ReLU.backward(dlinear, linear_relu_cache)
    dpool = FastMaxPool.backward(dlinear_relu, pool_cache)
    dconv, grads['W1'], grads['b1'] = FastConv.backward(dpool, conv_cache)
    loss += self.reg * (torch.sum(W3**2) + torch.sum(W2**2)+torch.sum(W1**2))
...
class DeepConvNet(object):
    ...
    for i in range(self.num_layers):
      if i == self.num_layers - 1:
        if weight_scale == 'kaiming':
          self.params[f'W{i+1}'] = kaiming_initializer(flatten, num_classes, K=None, relu=False, device=device, dtype=dtype)
        else:
          self.params[f'W{i+1}'] = weight_scale * torch.randn(flatten, num_classes, dtype=dtype, device=device)
        self.params[f'b{i+1}'] = torch.zeros(num_classes, dtype=dtype, device=device)
      else:
        F = num_filters[i]
        if weight_scale == 'kaiming':
          self.params[f'W{i + 1}'] = kaiming_initializer(F, C, K=3, relu=True, device=device, dtype=dtype)
        else:
          self.params[f'W{i+1}'] = weight_scale * torch.randn(F, C, 3, 3, dtype=dtype, device=device)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1, -1, -1):
      if i == self.num_layers-1:
        dhiddens[i], grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear.backward(dhiddens[i+1], cache_list[i])
      else:
        if i in self.max_pools:
          if self.batchnorm:
            dhiddens[i], grads[f'W{i+1}'], grads[f'b{i+1}'], grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = Conv_BatchNorm_ReLU_Pool.backward(dhiddens[i+1], cache_list[i])
          else:
            dhiddens[i], grads[f'W{i+1}'], grads[f'b{i+1}'] = Conv_ReLU_Pool.backward(dhiddens[i+1], cache_list[i])
        else:
          if self.batchnorm:
            dhiddens[i], grads[f'W{i + 1}'], grads[f'b{i + 1}'], grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = Conv_BatchNorm_ReLU.backward(dhiddens[i + 1], cache_list[i])
          else:
            dhiddens[i], grads[f'W{i+1}'], grads[f'b{i+1}'] = Conv_ReLU.backward(dhiddens[i+1], cache_list[i])
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      var_sqrt = (sample_var + eps).sqrt()
      x_normalized = x_mean / var_sqrt
      out = x_normalized * gamma + beta
      cache = mode, x_normalized, sample_mean, sample_var, x, eps, gamma, beta
      running_mean = momentum * running_mean + (1-momentum) * sample_mean
      running_var = momentum * running_var + (1-momentum) * sample_var
      ...
      var_sqrt = (running_var + eps).sqrt()
      x_normalized = x_mean / var_sqrt
      out = x_normalized * gamma + beta
      cache = mode, x_normalized, running_mean, running_var, x, eps, gamma, beta
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    else:
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    mode, x_normalized, sample_mean, sample_var, x, eps, gamma, beta = cache
    N, D = dout.shape
    if mode == 'train':
      dbeta = torch.sum(dout, dim=0)
      dgamma = torch.sum(x_normalized * dout, dim=0)
      dnormalized = dout * gamma
      dx_mean1 = dnormalized / ((sample_var + eps) ** 0.5)
      dvar_sqrt = - torch.sum((x - sample_mean) * dnormalized, dim=0) / (sample_var + eps)
      dvar_esp = 0.5 / ((sample_var + eps) ** 0.5) * dvar_sqrt
      dx_mean2 = 1 / N * dvar_esp * 2 * (x - sample_mean)
      dx_mean = dx_mean1 + dx_mean2
      dmean = - torch.sum(dx_mean, dim=0)
      dx = dnormalized / ((sample_var + eps) ** 0.5) + dvar_esp * 2 * (x - sample_mean) / x.shape[0] + dmean / x.shape[0]
  ...
  def backward_alt(dout, cache):
    ...
    if mode == 'train':
      dbeta = torch.sum(dout, dim=0)
      dgamma = torch.sum(x_normalized * dout, dim=0)
      dx_mean = dout * gamma / ((sample_var + eps) ** 0.5) + 1 / N * (0.5 / ((sample_var + eps) ** 0.5) * (- torch.sum((x - sample_mean) * dout * gamma, dim=0) / (sample_var + eps))) * 2 * (x - sample_mean)
      dmean = - torch.sum(dx_mean, dim=0)
      dx = dout * gamma / ((sample_var + eps) ** 0.5) + (0.5 / ((sample_var + eps) ** 0.5) * (- torch.sum((x - sample_mean) * dout * gamma, dim=0) / (sample_var + eps))) * 2 * (x - sample_mean) / x.shape[0] + dmean / x.shape[
        0]
