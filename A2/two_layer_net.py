...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    loss = torch.sum(-torch.log(score_softmax[range(N), y]))
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    score_softmax[range(N), y] -= 1
    score_softmax /= N
    grads['W2'] = h1.t().mm(score_softmax)
    grads['W2'] += 2 * reg * W2
    grads['b2'] = torch.sum(score_softmax, dim=0)
    dh1 = score_softmax.mm(W2.t())
    dh1[h1 == 0] = 0
    grads['W1'] = X.t().mm(dh1)
    grads['W1'] += 2 * reg * W1
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  y_pred = torch.max(loss_func(params, X), dim=1)[1]
  ###########################################################################
  #                              END OF YOUR CODE                           #
  ###########################################################################
      ...
      for lr in learning_rates:
          for reg in regularization_strengths:
             for lr_decay in learning_rate_decays:
                 print(f'train with hidden size: {hs} learning rate: {lr} regularization_strength: {reg} learning rate decay: {lr_decay}')
