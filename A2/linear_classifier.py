...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  activation = (margin > 0).to(torch.float64)
  sum_activation = torch.sum(activation, axis=1, dtype=torch.float64)
  activation[range(num_train), y] = -sum_activation
  dW += X.t().mm(activation)
  dW /= num_train
  dW += 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
  ...
  regularization_strengths = [3e-2, 6e-2, 1e-1, 3e-1]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
      scores = X[i].matmul(W)
      score_softmax = torch.softmax(scores, dim=0)
      loss += -torch.log(score_softmax[y[i]])
      dW += torch.outer(X[i], score_softmax)
      dW[:, y[i]] -= X[i]
  loss /= num_train
  loss += reg * torch.sum(W * W)
  dW /= num_train
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  score_softmax = torch.softmax(scores, dim=1)  # shape (N, C)
  loss += torch.sum(-torch.log(score_softmax[range(num_train), y]))
  score_softmax[range(num_train), y] -= 1
  dW += X.t().mm(score_softmax)
  loss /= num_train
  loss += reg * torch.sum(W * W)
  dW /= num_train
  dW += 2 * reg * W
  # print(dW[:, y].shape) # shape(D, N)
  #############################################################################
  #                          END OF YOUR CODE                                 #
  #############################################################################
